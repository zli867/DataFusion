...
def writeToNetCDF(data_fusion_results, pollutant_name, input_file_name, output_file_name):
    ...
    with netCDF4.Dataset(input_file_name) as src, netCDF4.Dataset(output_file_name, "w") as dst:
        ...
        for name, variable in src.variables.items():
            ...
            if name in ["time", "lev", "ilev", "lat", "lon", "hyam", "hybm", "hyai", "hybi", "P0"]:
                x = dst.createVariable(name, variable.datatype, variable.dimensions)
                dst[name].setncatts(src[name].__dict__)
                dst[name][:] = src[name][:]

# random withhoding
work_dir =  "/Volumes/DataStorage/DataFusionData/validation/GC/O3"
CMAQ_file = "/Volumes/DataStorage/DataFusionData/GC/GC_Combine_2015.nc4"
obs_file =  "/Volumes/DataStorage/DataFusionData/GC/O3_obs_2010_2019_CONUS_GC.csv"
CMAQ_pollutant = "O3_MDA8"
obs_pollutant = "O3"
k_fold = 5

geo = GCGridInfo(CMAQ_file)
CMAQList = extractCMAQ(CMAQ_pollutant, CMAQ_file, geo)

obs_df = pd.read_csv(obs_file, parse_dates=["Time"], date_parser=dateparse)
obs_df = obs_df[(obs_df["Time"] >= datetime(2015, 1, 1)) & (obs_df["Time"] <= datetime(2015, 12, 31))]
...
for i in range(0, k_fold):
    data_fused_filename = os.path.join(work_dir, "GC_Combine_2015_o3_fused_fold_" + str(i) + ".nc4")
...
# for i in range(0, k_fold):
#     data_fused_filename = os.path.join(work_dir, "GC_Combine_2015_o3_fused_fold_site_" + str(i) + ".nc")
#     train_obs_list = extractOBS(obs_pollutant, obs_train_filenames[i], geo)
#     datafusion_results = dataFusion(CMAQList, train_obs_list, geo, 'default')
#     writeToNetCDF(datafusion_results, CMAQ_pollutant, CMAQ_file, data_fused_filename)